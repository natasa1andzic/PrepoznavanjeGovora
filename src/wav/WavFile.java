...
public class WavFile
{
	...
	private static IOState ioState;				// Specifies the IO State of the Wav File (used for snaity checking)
	private static int bytesPerSample;			// Number of bytes required to store a single sample
	private static long numFrames;					// Number of frames within the data section
	private static FileOutputStream oStream;	// Output stream used for writting data
	private static FileInputStream iStream;		// Input stream used for reading data
	private static double floatScale;				// Scaling factor used for int <-> float conversion				
	private static double floatOffset;			// Offset factor used for int <-> float conversion				
	private static boolean wordAlignAdjust;		// Specify if an extra byte at the end of the data chunk is required for word alignment

	// Wav Header
	private static int numChannels;				// 2 bytes unsigned, 0x0001 (1) to 0xFFFF (65,535)
	...
	// Buffering
	private static byte[] buffer;					// Local buffer used for IO
	private static int bufferPointer;				// Points to the current position in local buffer
	private static int bytesRead;					// Bytes read after last read into local buffer
	private static long frameCounter;				// Current number of frames read or written
	...
	private static long readSample() throws IOException, WavFileException
	{
		long val = 0;
	...
	// Integer
	// -------
	public static int readFrames(int[] sampleBuffer, int numFramesToRead) throws IOException, WavFileException
	{
		return readFrames(sampleBuffer, 0, numFramesToRead);
	}

	public static int readFrames(int[] sampleBuffer, int offset, int numFramesToRead) throws IOException, WavFileException
	{
